...
void showStatusBar()
{
  ...
  display.drawString(78, 56, "2020-12-14");
}
//温度显示界面
void showTemp()
{
...
void upload_while_oled_on()
{
  if (workingState == WORKING)
  {
    ...
    if (now_rec_stamp - last_rec_stamp > rec_span)
    {
      ...
      for (size_t i = 0; i < 101; i++)
      {
        ...
        display.setFont(Roboto_Condensed_38);
        display.setTextAlignment(TEXT_ALIGN_CENTER);
        display.drawString(64, 5, "UPLOAD");
        display.drawProgressBar(5, 50, 118, 8, i);
        display.setTextAlignment(TEXT_ALIGN_LEFT);
        display.display();
        delay(50);
      }
...
void oled_on_off_switch()
{
  ...
  if (screen_On_now - screen_On_Start > screen_On_last_span + screen_Off_to_sleep_span)
  {
    if (workingState == WORKING)
    {
      esp_sleep_enable_timer_wakeup(rec_span * 1000);
    }
